...
class Path
{
	...
	static fromVertices(vertices: Coords[] )
	{
		return new Path(vertices);
	}

	static manyFromEdges(edges: Edge[] ): Path[]
	{
		...
		while (edgesToPutIntoPaths.length > 0)
		{
			...
			for (var i = 0; i < 1; i++)
			{
				...
				var pathEdgeToBeConnectedTo =
					pathCurrentAsEdges[e];

				var v = i;
				var vReversed = 1 - v;

				var pathVertexToBeConnectedTo =
					pathEdgeToBeConnectedTo.vertices[v];

				var edgeToBeConnectedToPath =
					edgesToPutIntoPaths
						.find
						(
							e => e.vertices.some(v => v.equals(pathVertexToBeConnectedTo) )
						);
				...
				if (edgeToBeConnectedToPath != null)
				{
					...
					var vertexToBeConnectedToPath =
						edgeToBeConnectedToPath.vertices[vReversed];

					var edgeNeedsReversing =
						(vertexToBeConnectedToPath.equals(pathVertexToBeConnectedTo) == false);
					...
					var edgeIndexToRemoveAt =
						edgesToPutIntoPaths.indexOf(edgeToBeConnectedToPath);
		...
		var paths =
			pathsSoFarAsEdgeArrays
				.map
				(
					p =>
						Path.fromVertices
						(
							p.map(e => e.vertices[0] )
						)
				); 
	...
	static fromStringVerticesAsXxY(pathAsString: string): Path
	{
